...
export const markLectureAsComplete = async (data, token) => {
  ...
  try {
    ...
    console.log("MARK_LECTURE_AS_COMPLETE ERROR:", error)
    toast.dismiss(toastId)
    toast.error(error.message)
    result = false
  }
...
export const searchCourse = async (searchQuery, page = 1, limit = 12) => {
  const toastId = toast.loading("Searching...")
  let result = { courses: [], pagination: {} }
  try {
    const response = await apiConnector("POST", SEARCH_COURSE_API, { searchQuery, page, limit })
    if (!response?.data?.success) throw new Error("Could Not Search Courses")
    result = { courses: response.data.data, pagination: response.data.pagination }
  } catch (error) {
